...
(function ($) {
    ...
    $.fn.mdSlider = function(options) {
                ...
                if (options.fullheight) {
                    $(this).find(".md-object").each(function() {
                        var top =  $(this).data("y") ? $(this).data("y") : 0,
                            ...
                            objectRatio = width / height;
                            ratio = options.width / options.height;
                            newRatio = $(window).width() / $(window).height();
                            newLeft = (left / options.width * 100) + "%";
                        if (newRatio < ratio) {
                            if (width > 0)
                                $(this).width(width / options.width * $(window).width());
                            if (width > 0) {
                                $(this).height($(this).width() / objectRatio);
                            }  
                        }
                        if (newRatio > ratio) {
                            if (height > 0)
                                $(this).height(height / options.height * $(window).height());
                            if (width > 0) {
                                $(this).width($(this).height() * objectRatio);
                            }   
                            newLeft = ((left + (width - $(this).width()) / 2) / options.width * 100) + "%";              
                        }
            ...
            if($(".md-video", wrap).size() > 0) {
				if(options.videoBox) {
					...
					$(".md-video", wrap).click(function() {
						...
						video_ele.attr('allowfullheight' , '').attr('frameborder' , '0').css({width:"100%", height: "100%", background: "black"});
            ...
            self.find('.' + options.itemClassName).each(function (index) {                
                ...
                $(this).find(".md-object").each(function() {
                    ...
                    var css = {                       
                        top:(top / options.height * 100) + "%",
                        left:newLeft
                        // left:((left + (width - $(this).width()) / 2) / options.width * 100) + "%"
                    };
                    $(this).css(css);
                });                
            });
        ...
        function resizeFontSize() {
            ...
            // fullscreen
            ratio = options.width / options.height;
            newRatio = wrap.width() / wrap.height();
            ...
            if (newRatio < ratio) {
                ...
                if (wrap.height() < options.height) {
                    $(".md-objects", self).css({'font-size': wrap.height() / options.height * 100 - fontDiff + '%'});
                } else {
                    $(".md-objects", self).css({'font-size': 100 - fontDiff + '%'});
                }
            }                    
        ...
        function resizePadding() {
            if (wrap.width() < options.width && options.responsive) {
                $(".md-objects div.md-object", self).each(function() {
                    ...
                    //     $_object.find('a')[0].style.setProperty('padding-top', objectPadding['padding-top'] + 'px', 'important');
                    //     $_object.find('a')[0].style.setProperty('padding-right', objectPadding['padding-right'] + 'px', 'important');
                    //     $_object.find('a')[0].style.setProperty('padding-bottom', objectPadding['padding-bottom'] + 'px', 'important');
                    //     $_object.find('a')[0].style.setProperty('padding-left', objectPadding['padding-left'] + 'px', 'important');
                    // } else {                        
                        // $_object.css(objectPadding);
                        $_object[0].style.setProperty('padding-top', objectPadding['padding-top'] + 'px', 'important');
                        $_object[0].style.setProperty('padding-right', objectPadding['padding-right'] + 'px', 'important');
                        $_object[0].style.setProperty('padding-bottom', objectPadding['padding-bottom'] + 'px', 'important');
                        $_object[0].style.setProperty('padding-left', objectPadding['padding-left'] + 'px', 'important');
                    // }
    ...
    $.fn.mdvideobox = function (opt) {
        $(this).each(function() {
            ...
            function insert()
            {
                ...
                embed = '<iframe src="' + videoSrc + '" width="' + options.contentsWidth + '" height="' + options.contentsHeight + '" frameborder="0" webkitAllowfullheight mozallowfullheight allowfullheight></iframe>';
